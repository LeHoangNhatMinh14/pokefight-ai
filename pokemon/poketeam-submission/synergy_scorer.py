...
class PokemonSynergyScorer:
    ...
    def __init__(self, pokemon_df: pd.DataFrame):
        ...
        self.role_synergy = {
            'fast attacker': {
                'fast attacker': 0.3,  # Some overlap but different speeds
                'physical attacker': 0.8,  # Good mix of physical/special
                'special attacker': 0.8,  # Good mix of physical/special
                'bulky defender': 0.9,  # Speed + bulk = good
                'defensive support': 0.7,  # Speed + support = good
                'balanced': 0.6  # Decent general synergy
            },
            'physical attacker': {
                'fast attacker': 0.8,
                'physical attacker': 0.4,  # Too much physical overlap
                'special attacker': 0.9,  # Perfect physical/special mix
                'bulky defender': 0.7,
            ...
            'special attacker': {
                ...
                'special attacker': 0.4,  # Too much special overlap
                'bulky defender': 0.7,
                'defensive support': 0.6,
                'balanced': 0.7
            },
            'bulky defender': {
                ...
                'bulky defender': 0.5,  # Too much bulk
                'defensive support': 0.8,  # Bulk + support = wall team
                'balanced': 0.6
            },
            'defensive support': {
                'fast attacker': 0.7,
                ...
                'bulky defender': 0.8,
                'defensive support': 0.5,  # Too much support
                'balanced': 0.7
            },
            'balanced': {
                'fast attacker': 0.6,
    ...
    def calculate_type_synergy(self, pokemon1: Dict, pokemon2: Dict) -> float:
        ...
        Args:
            pokemon1, pokemon2: Pokemon dicts with 'type1', 'type2', 'weaknesses' keys

        Returns:
            Synergy score between 0-1
        """
        p1_weaknesses = set(pokemon1['weaknesses'])
        p2_weaknesses = set(pokemon2['weaknesses'])

        # Types that pokemon1 is weak to but pokemon2 resists
        p1_covered = len(p1_weaknesses.intersection(pokemon2.get('resistances', set())))
        # Types that pokemon2 is weak to but pokemon1 resists
        p2_covered = len(p2_weaknesses.intersection(pokemon1.get('resistances', set())))

        # Bonus for shared resistances (both resist the same types)
        p1_resistances = set()
        p2_resistances = set()

        for atk_type in self.type_effectiveness.keys():
            p1_mult = self.get_type_multiplier(atk_type, pokemon1['type1'], pokemon1.get('type2'))
            p2_mult = self.get_type_multiplier(atk_type, pokemon2['type1'], pokemon2.get('type2'))

            if p1_mult < 1:
                p1_resistances.add(atk_type)
            if p2_mult < 1:
                p2_resistances.add(atk_type)

        shared_resistances = len(p1_resistances.intersection(p2_resistances))

        # Calculate synergy score
        coverage_score = (p1_covered + p2_covered) / max(1, len(p1_weaknesses) + len(p2_weaknesses))
        resistance_bonus = shared_resistances * 0.1  # Small bonus for shared resistances
    ...
    def calculate_stat_synergy(self, team_stats: List[Dict]) -> float:
        ...
        Args:
            team_stats: List of stat dicts for team members

        Returns:
            Balance score between 0-1 (1 = perfectly balanced)
        """
        if len(team_stats) < 2:
            return 1.0

        # Calculate average stats across team
        stat_names = ['hp', 'attack', 'defense', 'sp_attack', 'sp_defense', 'speed']
        team_averages = {}

        for stat in stat_names:
            values = [member.get(stat, 0) for member in team_stats]
            team_averages[stat] = sum(values) / len(values)
        ...
        # Scale so that variance of 0 = 1.0, variance of 1000 = 0.0
        balance_score = max(0, 1 - (avg_variance / 1000))
    ...
    def score_team_synergy(self, team: List[Dict]) -> Dict[str, float]:
        ...
        Returns:
            Dict with synergy scores: 'type_synergy', 'role_synergy', 'stat_balance', 'overall'
        """
        if len(team) < 2:
    ...
    def score_candidate_for_team(self, candidate: Dict, current_team: List[Dict]) -> Dict[str, float]:
        ...
        Args:
            candidate: Pokemon dict to score
            current_team: List of current team Pokemon dicts

        Returns:
            Dict with scores: 'type_synergy', 'role_synergy', 'weakness_coverage', 'overall'
        """
        if not current_team:
            return {'type_synergy': 1.0, 'role_synergy': 1.0, 'weakness_coverage': 1.0, 'overall': 1.0}

        # Calculate synergies with each team member
        type_synergies = []
        role_synergies = []
        ...
        for member in current_team:
            ...
            # Track weakness coverage
            member_weaknesses = set(member['weaknesses'])
            team_weaknesses.update(member_weaknesses)

            # Check if candidate covers any team weaknesses
            candidate_resistances = set()
            for atk_type in self.type_effectiveness.keys():
                mult = self.get_type_multiplier(atk_type, candidate['type1'], candidate.get('type2'))
                if mult < 1:
                    candidate_resistances.add(atk_type)

            covered_weaknesses += len(member_weaknesses.intersection(candidate_resistances))
