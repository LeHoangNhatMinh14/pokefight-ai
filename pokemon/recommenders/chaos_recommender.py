...
import sys
# This file lives at <repo>/recommenders/chaos_recommender.py, so we add the
# repo root (parent.parent) to sys.path to expose the sibling top-level
# packages `data_pipeline` and `model` when this module is run as a script.
...
class ChaosRecommender:
    ...
    def build_team(
        ...
        *,
        ...
        _t_start = time.time()
        logger.info(
            "build_team favorite=%s gen=%s tier=%s team_size=%d max_leg=%s",
            favorite, gen, tier, team_size, max_legendaries,
        )
        ...
        logger.info(
            "build_team done favorite=%s gen=%s tier=%s viability=%.3f duration=%.2fs team=%s",
            favorite, gen, tier, best_score, time.time() - _t_start,
            ",".join(best_team),
        )
